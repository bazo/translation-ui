  ...
  foreach ($document as $key => $value) {
    if (is_array($value)) {
      if (isset($value['$ref'])) {
        ...
        $ref = findMongoDbDocument($mongo, $value['$id'], $_REQUEST['db'], $value['$ref']);
        if (!$ref) {
          $ref = findMongoDbDocument($mongo, $value['$id'], $_REQUEST['db'], $value['$ref'], true);
        }

        $refDb = isset($value['$db']) ? $value['$db'] : $_REQUEST['db'];

        $document[$key]['$ref'] = '<a href="'.$baseUri.'?db='.$refDb.'&collection='.$value['$ref'].'">'.$value['$ref'].'</a>';

        if ($ref['_id'] instanceof MongoId) {
          $document[$key]['$id'] = '<a href="'.$baseUri.'?db='.$refDb.'&collection='.$value['$ref'].'&id='.$value['$id'].'">'.$value['$id'].'</a>';
        } else {
          $document[$key]['$id'] = '<a href="'.$baseUri.'?db='.$refDb.'&collection='.$value['$ref'].'&id='.$value['$id'].'&custom_id=1">'.$value['$id'].'</a>';
        }

        if (isset($value['$db'])) {
            $document[$key]['$db'] = '<a href="'.$baseUri.'?db='.$refDb.'">'.$refDb.'</a>';
        }
      } else {
 ...
 *
 */
function findMongoDbDocument($mongo, $id, $db, $collection, $forceCustomId = false)
{
...
try {
  ...
  if (isset($_REQUEST['search']) && !is_object(json_decode($_REQUEST['search']))) {
    $customId = false;
    $document = findMongoDbDocument($mongo, $_REQUEST['search'], $_REQUEST['db'], $_REQUEST['collection']);

    if (!$document) {
      $document = findMongoDbDocument($mongo, $_REQUEST['search'], $_REQUEST['db'], $_REQUEST['collection'], true);
      $customId = true;
    }

    if (isset($document['_id'])) {
      $url = $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection'] . '&id=' . (string) $document['_id'];
  ...
  if (isset($_REQUEST['create_db']) && $readOnly !== true) {
    ...
    header('location: ' . $baseUri . '?db=' . $_REQUEST['create_db']);
    exit;
  ...
  if (isset($_REQUEST['create_collection']) && $readOnly !== true) {
	...
	$this->log(sprintf('created collection: %s', $_REQUEST['create_collection']));
    header('location: ' . $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['create_collection']);
    exit;
  }
  ...
  if (isset($_REQUEST['delete_collection']) && $readOnly !== true) {
    $mongo
      ...
      ->drop();
	$this->log(sprintf('deleted collection: %s', $_REQUEST['delete_collection']));
    header('location: ' . $baseUri . '?db=' . $_REQUEST['db']);
    exit;
  }
  ...
  if (isset($_REQUEST['delete_document']) && $readOnly !== true) {
	...
	$this->log(sprintf('deleted document: id: %s from collection: %s', $_REQUEST['delete_document'], $_REQUEST['collection']));
    header('location: ' . $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection']);
    exit;
  }
  ...
  if (isset($_REQUEST['delete_document_field']) && $readOnly !== true) {
    ...
    $document = findMongoDbDocument($mongo, $_REQUEST['id'], $_REQUEST['db'], $_REQUEST['collection']);
    unset($document[$_REQUEST['delete_document_field']]);
    $coll->save($document);

	$this->log(sprintf('deleted field from document: field: %s, id: %s from collection: %s', $_REQUEST['delete_document_field'],$_REQUEST['id'], $_REQUEST['collection']));
  ...
  if (isset($_POST['save']) && $readOnly !== true) {
	...
	$this->log(sprintf('edited document: data: %s, in collection: %s', $_REQUEST['value'], $_REQUEST['collection']));
...
<?php if ( ! isset($_REQUEST['db'])): ?>
  ...
  <table class="table table-bordered table-striped">
    ...
    <tbody>
      ...
      <?php foreach ($dbs['databases'] as $db): if ($db['name'] !== $container->parameters['database']['dbname']) continue; ?>
        <tr>
          <td><a href="<?php echo $baseUri . '?db=' . $db['name'] ?>"><?php echo $db['name'] ?></a></td>
          <td><?php echo count($mongo->selectDb($db['name'])->listCollections()) ?></td>

		  <?php /*
		  
          <?php if ($readOnly !== true): ?>
            <td><a href="<?php echo $baseUri ?>?delete_db=<?php echo $db['name'] ?>" onClick="return confirm('Are you sure you want to delete this database?');">Delete</a></td>
          <?php else: ?>
            <td>&nbsp;</td>
          <?php endif; ?>
		*/	
...
<?php elseif (isset($_REQUEST['db']) && ! isset($_REQUEST['collection'])): ?>
  ...
  <?php if ($readOnly !== true): ?>
    <div id="create_form">
      <form action="<?php echo $baseUri ?>?db=<?php echo $_REQUEST['db'] ?>" method="POST">
  ...
  <h2>
    <a href="<?php echo $baseUri ?>">Databases</a> >>
  ...
  <table class="table table-bordered table-striped">
    ...
    <tbody>
      ...
      <?php foreach ($collections as $collection): ?>
        <tr>
          <td><a href="<?php echo $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $collection->getName() ?>"><?php echo $collection->getName() ?></a></td>
          <td><?php echo $collection->count(); ?></td>

         <?php if ($readOnly !== true): ?>
            <td><a href="<?php echo $baseUri ?>?db=<?php echo $_REQUEST['db'] ?>&delete_collection=<?php echo $collection->getName() ?>" onClick="return confirm('Are you sure you want to delete this collection?');">Delete</a></td>
          <?php else: ?>
            <td>&nbsp;</td>
          <?php endif; ?>
        </tr>
...
<?php elseif ( ! isset($_REQUEST['id']) || isset($_REQUEST['search'])): ?>
    ...
    <h2>
      <a href="<?php echo $baseUri ?>">Databases</a> >>
      <a href="<?php echo $baseUri ?>?db=<?php echo $_REQUEST['db'] ?>"><?php echo $_REQUEST['db'] ?></a> >>
    ...
    <?php if ($pages > 1): ?>
      <div id="pager">
        ...
        <input type="text" name="page" size="4" value="<?php echo $page ?>" onChange="javascript: location.href = '<?php echo $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection'] ?><?php if (isset($_REQUEST['search'])): ?>&search=<?php echo urlencode($_REQUEST['search']) ?><?php endif; ?>&page=' + this.value;" />
        <input type="button" name="go" value="Go" />
      </div>
    <?php endif; ?>

    <div id="search">
      <form action="<?php echo $baseUri ?>" method="GET">
        ...
        <?php foreach ($cursor as $document): ?>
          <tr>
            <?php if (is_object($document['_id']) && $document['_id'] instanceof MongoId): ?>
              <td><a href="<?php echo $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection'] ?>&id=<?php echo (string) $document['_id'] ?>"><?php echo (string) $document['_id'] ?></a></td>
            <?php else: ?>
              <td><a href="<?php echo $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection'] ?>&id=<?php echo (string) $document['_id'] ?>&custom_id=1"><?php echo (string) $document['_id'] ?></a></td>
            <?php endif; ?>
            <td>
              <?php
                if (isset($search)) {
            ...
            <?php if (is_object($document['_id']) && $document['_id'] instanceof MongoId && $readOnly !== true): ?>
              <td><a href="<?php echo $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection'] ?>&delete_document=<?php echo (string) $document['_id'] ?>" onClick="return confirm('Are you sure you want to delete this document?');">Delete</a></td>
            <?php elseif ($readOnly !== true): ?>
              <td><a href="<?php echo $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection'] ?>&delete_document=<?php echo (string) $document['_id'] ?>&custom_id=1" onClick="return confirm('Are you sure you want to delete this document?');">Delete</a></td>
            <?php endif; ?>
          </tr>
        <?php endforeach; ?>
      </tbody>
...
<h2>
    <a href="<?php echo $baseUri ?>">Databases</a> >>
    <a href="<?php echo $baseUri ?>?db=<?php echo $_REQUEST['db'] ?>"><?php echo $_REQUEST['db'] ?></a> >>
    <a href="<?php echo $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection'] ?>"><?php echo $_REQUEST['collection'] ?></a> >>
    <?php echo $_REQUEST['id'] ?>
    </h2>
    <?php $document = findMongoDbDocument($mongo, $_REQUEST['id'], $_REQUEST['db'], $_REQUEST['collection']); ?>
    ...
    <br/>
    <?php if (is_object($document['_id']) && $document['_id'] instanceof MongoId && $readOnly !== true): ?>
      <a class="save_button" href="<?php echo $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection'] ?>&delete_document=<?php echo (string) $document['_id'] ?>" onClick="return confirm('Are you sure you want to delete this document?');">Delete</a>
    <?php elseif ($readOnly !== true): ?>
      <a class="save_button" href="<?php echo $baseUri . '?db=' . $_REQUEST['db'] . '&collection=' . $_REQUEST['collection'] ?>&delete_document=<?php echo (string) $document['_id'] ?>&custom_id=1" onClick="return confirm('Are you sure you want to delete this document?');">Delete</a>
    <?php endif; ?>
