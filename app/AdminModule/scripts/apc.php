...
function menu_entry($ob,$title, $MYREQUEST, $MY_SELF) {
	...
	{
		return '<li class=active><a href="#">'.$title.'</span></li>';
	} else 
	{
...
echo
	...
	menu_entry(4,'Per-Directory Entries', $MYREQUEST, $MY_SELF),
...
case OB_HOST_STATS:
	...
	echo <<< EOB
		...
		<table class="table table-bordered table-striped" cellspacing=0><tbody>
...
EOB;
    ...
    <tr class=tr-0><td class=td-0>Cached Variables</td><td>$number_vars ($size_vars)</td></tr>
		...
		<div class="info div2"><h2>Runtime Settings</h2><table class="table table-bordered table-striped" cellspacing=0><tbody>
EOB;
...
EOB;
	...
	echo
		graphics_avail() ? 
			  '<tr>'.
			  "<td class=td-0><img alt=\"\" $size src=\"$PHP_SELF?IMG=1&t=$time\"></td>".
			  "<td class=td-1><img alt=\"\" $size src=\"$PHP_SELF?IMG=2&t=$time\"></td></tr>\n"
			: "",
		'<tr>',
...
EOB;
	...
	if (graphics_avail()) {
		...
		echo <<<EOB
			<img alt="" $size src="$PHP_SELF?IMG=3&t=$time">
EOB;
	}
	echo <<<EOB
...
case OB_SYS_CACHE:	
	...
	{
		echo <<< EOB
			<div class="info"><table class="table table-bordered table-striped" cellspacing=0><tbody>
